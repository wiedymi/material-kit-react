...
const TattooList = (props) => {
  ...
  const { token } = props;
  const [tattoosList, setTattoosList] = useState(null);
    ...
    observer.current = new IntersectionObserver(entries => {
      if(entries[0].isIntersecting) {
        if(hasMoreRef.current) {
            fetch(`${baseUrl}/designs/?name_icontains=${searchRef.current}&page=${pageRef.current}`, {
            headers: {
              ...baseHeaders,
  ...
  const searchName = () => {
    pageRef.current = 1;
    fetch(`${baseUrl}/designs/?name__icontains=${search}&page=${page}`, {
      headers: {
        ...baseHeaders,
        Authorization: `Bearer ${token}`
      }
      ...
      <Container maxWidth={false} onScroll={() => handleScroll()}>
        ...
        <Box>
            <div className={classes.ImageGrid}>
              {tattoos.length > 0 && tattoos.map((tattoo, idx) => {
                var height = _.sample(gridHeights);
                if(tattoos.length === idx + 1) {
                  return(
                      <div ref={lastElementRef} style={{gridRowEnd: `span ${Math.ceil(height / 8)}`}} key={idx}>
                        <TattooCard tattoo={tattoo} height={height} design={true}/>
                      </div>
                  )
                }
                return(
                  <div style={{gridRowEnd: `span ${Math.ceil(height / 8)}`}} key={idx}>
                    <TattooCard tattoo={tattoo} height={height} design={true}/>
                  </div>
              )})}
            </div>
...
export default connect(mapStateToProps)(TattooList);
